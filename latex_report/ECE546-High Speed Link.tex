...
\renewcommand{\section}[1]{\clearpage\psection{#1}}
...
As for package, we use one layer of thin-film ceramic from Murawa \cite{na_alumina_substrates}. The substrate is far more thinner than those used in PCB. Table.~1 shows the material properties used for PCB and package.
...
\begin{table}[h]
	...
	\begin{center}
		\begin{tabular}{| l | l | l | c | c | c |}
			...
			Package & Alumina \cite{na_alumina_substrates} & Copper & 2 & 9.8 & 0.02 \\
			\hline
		\end{tabular}
	\end{center}	
...
\begin{table}[htbp!]
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}	
	\caption{Design parameter of PCB trace.}\label{table:pkg_trace}
	\vskip0.2in
\end{table}
...
\label{sec:bond_wire}
The transition between package and PCB is through using bonding wire. Fig.~\ref{fig:bonding_wire_HFSS} shows the transition structure of bonding wire, and Table.~\ref{table:bonding_wire_parameter} shows the corresponding design parameter. Similar to via transition in PCB described in Sec.\ref{subsec:via_tran}, the lumped model shown in Fig.~\ref{fig:pcb_via_lump_model} also applied here. However, since the bonding wire is a very resistive and inductive, the additional patches were attached at the both side of bonding wire in order to decrease the differential impedance. Same equation in (\ref{eq:pcb_via_tran_lump_diff}) can also be applied here. 

Fig.~\ref{fig:bonding_wire_S} shows the S parameter of both differential and common mode, and Fig.\ref{fig:bonding_wire_S_smith} also shows the $S^{dd}_{11}$ and $S^{dd}_{22}$ on Smith chart. It can be observed that the reflection of differential modes from both ports have 0.1 reflection at 3GHz, whereas half the reflection at 10GHz. The high reflection at high frequency is because adding patches can only be affective at low frequency. In \cite{1IWSPI2S2_FikarS_ScholtzAL_2008_100ghz_bandwidth}, removing ground of both package and PCB will achieve better performance. 
...
\begin{figure*}[htbp!]
	...
	\end{minipage}	
	\caption{Bonding wire transition simulated in HFSS.}\label{fig:bonding_wire_HFSS}
\end{figure*}
...
\begin{table}[h]
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}	
	\caption{Design parameter of bonding wire transition.}\label{table:bonding_wire_parameter}
	\vskip0.2in
\end{table}
...
\begin{figure*}[htbp!]
	...
	\end{minipage}	
	\caption{Smith chart of Sdd11 and Sdd22 of bonding wire transition.}\label{fig:bonding_wire_S_smith}
\end{figure*}
...
\subsection{Channel Response without Equalization}
\label{subsec:channel_response}
After obtaining S-parameter for the entire channel from ADS cascade, we convert our 4 port S-parameters to Mixed Mode to observe the differential mode and common mode S-parameters respectively. Since our channel is designed to transmit differential signal, we want $S^{dd}_{11}$ to be as small as possible to minimize reflection and $S^{dd}_{12}$ to be as large as possible to minimize signal loss. Fig.~\ref{fig:Mixed Mode S-parameter Response} (a) confirms that our channel achieve the our goal and have very good performance.

In order to see the eye diagram and transient response for our channel at different bit rate, we also simulated our channel in Cadence Virtuoso with pseudo random bit sequence input for a long period of time. Fig.~\ref{fig:input_differential_eye} and Fig.~\ref{fig:output_differential_eye} shows the eye diagram of the input and output of the channel at 3 Gb/s and 20 Gb/s whereas Fig.~\ref{fig:input_transient_response} and Fig.~\ref{fig:output_transient_response} shows the transient response.
...
One of the FIR equalizer usually used is FFE. Since the FFE needs the "future" data bit, it is usually implemented in transmitted port. Fig.~\ref{fig:sampled_channel_response} shows the sampled plain channel differential response at received port. The discrete value of the sampled signal is listed at Table.~\ref{table:3G_rxdiff_sample_w__FFE} and Table.~\ref{table:20G_rxdiff_sample_w__FFE}. From Fig.~\ref{fig:sampled_channel_response} we can observe that there are  two precursors in differential pulse response. Therefore, we will use two taps $b_{-2},b_{-1},b_0$ to eliminate it. Follow similar procedure described in \cite{na_microsoft_powerpoint}, we may obtain the coefficient of each taps in FFE. 
...
\begin{figure*}[htbp!]
	...
	\label{fig:sampled_channel_response}
\end{figure*}
...
\begin{figure*}[htbp!]	
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{20 Gb/s. }
	\end{minipage}%
	\caption{Terminal signal after channel w/ and w/o FFE.}
	\label{fig:diff_pulse_response_w_FFE}
\end{figure*}
...
\begin{figure*}
	...
	\begin{minipage}[h]{\textwidth}
		...
		\end{tabular} 		
		\subcaption{Sampled signal of received differential w/o FFE.}\label{table:3G_rxdiff_sample_wo_FFE}
	\end{minipage}	
	\begin{minipage}[tb]{\textwidth}
		\centering	
		\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}\hline
		...
		\end{tabular}
		\subcaption{Sampled signal of received differential with FFE.}\label{table:3G_rxdiff_sample_w__FFE} 
	\end{minipage}
...
\begin{figure*}
	...
	\begin{minipage}[tb]{\textwidth}
		...
		\end{tabular}		
		\subcaption{Sampling signal of received differential w/o FFE.}\label{table:20G_rxdiff_sample_wo_FFE}
	\end{minipage}	
	\begin{minipage}[tb]{\textwidth}
		\centering	
		\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}\hline
		...
		\end{tabular}
		\subcaption{Sampling signal of received differential with FFE.}\label{table:20G_rxdiff_sample_w__FFE} 
	\end{minipage}
...
\begin{figure*}
	...
	\label{table:rx_diff_sample_DFE}
\end{figure*}
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{Example eye diagram of full-delay scheme of DFE.}
		\label{fig:dfe_eye_full}
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/Verilog/dfe/half_dfe.png}
		\subcaption{Example eye diagram of half-delay scheme of DFE propsed in \cite{na_energy_efficient, na_microsoft_powerpoint}.}
...
In Fig.~\ref{fig:diff_pulse_response_w_FFE} and Table.~\ref{table:rx_diff_sample_DFE} shows the sampled differential channel response with FFE enabled. These channel responses can further be used to determine the 5-tap DFE. Fig.~\ref{fig:dfe_pulse_response} shows the pulse response after applying the DFE. We can see that the pulse response abruptly drop or elevated at half of UI, so that the sampled signal will have no post cursors. Table.~\ref{table:rx_diff_sample_DFE} also listed the sampled signal after DFE.

\begin{figure*}[htbp!]
	\captionsetup[subfigure]{justification=centering}
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/Verilog/3G/2_diff_w_DFE.png}
		\subcaption{3Gb/s.}		
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/Verilog/20G/2_diff_w_DFE.png}
		\subcaption{20Gb/s.}
	\end{minipage}%	
	\caption{Pulse response after DFE.}\label{fig:dfe_pulse_response}
\end{figure*}
