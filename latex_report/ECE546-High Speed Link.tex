...
In the TX block, signal path is composed of a data transmitter and a feed-forward equalizer (FFE) implemented with Verilog, whereas the clock uses a clock . Both transmitters can be implemented using transistor level inverters. \\


\begin{figure}[htbp!]
	\centering
	\includegraphics[width=0.9\columnwidth]{./img/system_box_model.png}
	\caption{Block diagram of entire high speed data link.}
\end{figure}
...
\subsection{Plain Channel Response}
\highlight{frequency domain result :ADS, Sdd11 good, Sdd12 good, ...}
\hightlight{time domain result: }
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{20 Gb/s. }
	\end{minipage}
	\caption{Received transient terminal response.}
