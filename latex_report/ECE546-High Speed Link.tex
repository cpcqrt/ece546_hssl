...
\begin{figure}[htbp!]
	...
	\caption{PCI Express Data Rate in Different Generations.}
\end{figure}
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{Input Transient Response. }
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[height=0.15\textheight]{./img/channel_response_eye_diagram/output_transient_response_3gbp.png}
		\subcaption{Output Transient Response. }
	\end{minipage}
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{Output Eye Diagram. }
	\end{minipage}
	\caption{Eye diagram at 20 Gbps.}
\end{figure*}
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/channel_response_eye_diagram/input_transient_response_20gbp.png}
		\subcaption{Input Transient Response. }
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		...
		\subcaption{Output Transient Response. }
	\end{minipage}
	\caption{Transient Response at 20 Gbps.}
\end{figure*}
...
To further boost the performance of our high speed link, we add 

\highlight{Add some description on why we need FFE and DFE equalizers}

\cphighlight{Add reference to MATLAB}
We first use MATLAB to design the DFE and FFE, then validate our results in Sec.~\ref{sec:result}. 


\subsection{Feed Forward Equalizer}
\label{sec:FFE}
...
\begin{figure*}[htbp!]
	...
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{Sampled received differential channel response w/o FFE.}
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/Verilog/20G/1.png}
		\subcaption{Differential signal before channel w/ and w/o FFE.}
	\end{minipage}%
\end{figure*}	
\begin{figure*}[htbp!]	
	\ContinuedFloat
	\begin{minipage}[tb]{0.5\textwidth}
		...
		\subcaption{Received differential channel response w/o or w/o FFE.}
	\end{minipage}%	
	\begin{minipage}[tb]{0.5\textwidth}
		\centering	
		\includegraphics[width=\textwidth]{./img/Verilog/20G/2.png}
		\subcaption{Differential signal after channel w/ and w/o FFE.}
	\end{minipage}%
	\caption{Transient Response at 3 Gbps.}
\end{figure*}
...
Decision feedback equalizer (DFE) is a Finite Impulse Response (FIR) filter based feedback scheme that reduces Intersymbol Interference (ISI). DFE consists of a slicer (Decision unit) and a FIR filter. The slicer makes symbol decisions and determine whether the FIR filter should be used to reduce the postcursors. By analyzing the input differential signal, we can decide postcursors magnitude precisely. Postcursors can then be reduced by making filter tap coefficients equal to postcursor magnitude but with an opposite sign. The period between each symbol needs to be multiples of sampling period so that a correct decision can be made.
...
\cphighlight{Add Eyes of 1) pure channel, 2) with FFE, 3) with DFE}
...
http://www.cnet.com/uk/news/double-speed-next-gen-version-cements-pcie-computing-power/
%\end{thebibliography}
